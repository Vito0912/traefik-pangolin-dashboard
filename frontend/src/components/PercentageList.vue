      ...
      >
        <div class="flex-1 min-w-0">
          <div class="flex items-center justify-between mb-1">
            <span
              class="text-sm font-medium text-gray-700 truncate mr-2 min-w-0 flex-1"
              :title="item.name"
              >{{ item.name }}</span
            >
            <div class="flex items-center space-x-2 flex-shrink-0">
              <span class="text-sm font-semibold text-gray-900">{{ formatValue(item.value) }}</span>
              <span class="text-xs text-gray-500 whitespace-nowrap">({{ item.percentage }}%)</span>
            </div>
          </div>
        </div>
      </div>
